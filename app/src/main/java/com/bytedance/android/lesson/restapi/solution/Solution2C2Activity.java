...
public class Solution2C2Activity extends AppCompatActivity {
    ...
    private void initRecyclerView() {
        ...
        mRv.setAdapter(new RecyclerView.Adapter() {
            ...
            public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int i) {
                ...
                // endTODO-C2 (10) Uncomment these 2 lines, assign image url of Feed to this url variable
                String url = mFeeds.get(i).getImgUrl();
                iv.setImageResource(R.drawable.ic_launcher_background);
                Log.d(TAG, "onBindViewHolder: Glide Request");
    ...
    public void chooseImage() {
        // endTODO-C2 (4) Start Activity to select an image
        //华为手机在这里MIME设成*/*都没用，每次Intent.ACTION_GET_CONTENT都是"最近"
        //所以都改成ACTION_PICK了，可以自己选择app浏览文件
...
//        }
    ...
    private void postVideo() {
        ...
        // endTODO-C2 (6) Send Request to post a video with its cover image
        // if success, make a text Toast and show
        PostVideoThread thread = new PostVideoThread();
        thread.start();
    }
    ...
    public void fetchFeed(View view) {
        ...
        // endTODO-C2 (9) Send Request to fetch feed
        // if success, assign data to mFeeds and call mRv.getAdapter().notifyDataSetChanged()
        // don't forget to call resetRefreshBtn() after response received
        FetchFeedThread thread = new FetchFeedThread();
        thread.start();
    }

    class FetchFeedThread extends Thread{
        @Override
        public void run() {
            super.run();
            Retrofit retrofit = new Retrofit.Builder()
                    ...
                    .build();
            try {
                Log.d(TAG, "run: begin Fetch Feed");
                ...
                mRv.post(new Runnable() {
                    ...
                    public void run() {
                        if(response.body().isSuccess()) {
                            ...
                            Log.d(TAG, "run: Notify DataSet Changed");
                        }else{
                ...
                Log.d(TAG, "FetchFeedThread: "+e.getClass().getName()+" "+e.getMessage());
            }finally {
    ...
    class PostVideoThread extends Thread{
        @Override
        public void run() {
            super.run();
            Retrofit retrofit = new Retrofit.Builder()
                    ...
                    .build();
            try {
                Log.d(TAG, "run: begin Post");
                final Response<PostVideoResponse> response=
                        retrofit.create(IMiniDouyinService.class)
                        .postVideoResponse(9527,
                                "NiBuRenShiWo",
                                getMultipartFromUri("cover_image",mSelectedImage),
                                getMultipartFromUri("video",mSelectedVideo))
                        .execute();
                Log.d(TAG, "run: end Post");
                mRv.post(new Runnable() {
                    @Override
                    public void run() {
                        String toastmsg = "post failed";
                        if(response.body().isSuccess()){
                            toastmsg = "post succeed";
                        }
                        Toast.makeText(Solution2C2Activity.this,
                                toastmsg,
                                Toast.LENGTH_SHORT).show();
                    }
                });
            }catch (final Exception e){
                Log.d(TAG, "PostVideoThread: "+e.getClass().getName()+" "+e.getMessage());
                mBtnRefresh.post(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(Solution2C2Activity.this,
