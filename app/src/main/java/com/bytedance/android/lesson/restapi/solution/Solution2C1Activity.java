...
public class Solution2C1Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mRv.setAdapter(new Adapter() {
            ...
            public void onBindViewHolder(@NonNull ViewHolder viewHolder, int i) {
                ...
                // endTODO-C1 (4) Uncomment these 2 lines, assign image url of Cat to this url variable
    ...
    public void requestData(View view) {
        ...
        // endTODO-C1 (3) Send request for 5 random cats here, don't forget to use {@link retrofit2.Call#enqueue}
        // Call restoreBtn() and loadPics(response.body()) if success
        // Call restoreBtn() if failure
        AskCatsThread thread = new AskCatsThread();
        thread.start();
    ...
    class AskCatsThread extends Thread{
        @Override
        public void run() {
            super.run();
            Retrofit retrofit = new Retrofit.Builder()
                    ...
                    .build();
            try {
                Log.d(TAG, "requestData: before");
                Response<List<Cat>> catListResponse = retrofit.
                        create(ICatService.class).
                        requestCatList(5).
                        execute();
                Log.d(TAG, "requestData: after");
                if(catListResponse!=null){
                    final List<Cat> cats = (catListResponse.body());
                    Log.d(TAG, "requestData: setCats number="+cats.size());
                ...
                Log.d(TAG, "requestData: "+e.getClass().getName()+" "+e.getMessage());
            }finally {
                mBtn.post(new Runnable() {
                    @Override
                    public void run() {
